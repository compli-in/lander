          ...
          {posts.length === 0 ? (
            ...
            <div className="space-y-8">
              {posts.map((post) => (
                <article
                  key={post.slug}
                  className="group border border-slate-200 rounded-2xl p-8 hover:border-slate-300 hover:shadow-md transition-all"
                >
                  <Link href={`/blog/${post.slug}`}>
                    ...
                    <h2 className="text-2xl font-semibold text-slate-900 mb-3 group-hover:text-sky-500 transition-colors">
                      {post.title}
                    </h2>
